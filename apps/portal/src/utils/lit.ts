...
export async function signInWithGoogle(redirectUri: string): Promise<void> {
  const googleProvider = litAuthClient.initProvider<GoogleProvider>(ProviderType.Google, {
    redirectUri,
  })
...
export async function signInWithDiscord(redirectUri: string): Promise<void> {
  const discordProvider = litAuthClient.initProvider<DiscordProvider>(ProviderType.Discord, {
    redirectUri,
  })
 ...
 */
export async function mintPKP(authMethod: AuthMethod): Promise<IRelayPKP | null> {
  const provider = getProviderByAuthMethod(authMethod)
  ...
  if (authMethod.authMethodType === AuthMethodType.WebAuthn) {
    ...
    // Verify registration and mint PKP through relay server
    txHash = await (provider as WebAuthnProvider).verifyAndMintPKPThroughRelayer(
      webAuthnInfo,
      options,
    )
  } else {
