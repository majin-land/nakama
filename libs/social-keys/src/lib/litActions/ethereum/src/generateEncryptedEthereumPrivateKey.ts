...
(async () => {
  ...
  const payload = await nip04.decrypt(nostrPrivateKey, nostrRequest.pubkey, nostrRequest.content);
  ...
  const sig = await Lit.Actions.signAndCombineEcdsa({
    toSign,
    publicKey: pkpPublicKey, // @JsParams pkpPublicKey
  ...
  const nostrReply = {
    ...
    content: nip04.encrypt(nostrPrivateKey, nostrRequest.pubkey, nostrReplyMessage),
  }
