...
import { uint8arrayFromString, uint8ArrayToBase64 } from '@lit-protocol/uint8arrays'

import { LIT_SESSIONSIG_AUTHORIZATION_SCHEMA_PREFIX, SERVICE_URL_BY_LIT_NETWORK } from './constants'
...
function composeAuthHeader(sessionSig: AuthSig) {
  ...
  return `${LIT_SESSIONSIG_AUTHORIZATION_SCHEMA_PREFIX}${uint8ArrayToBase64(sessionSigUintArr)}`
}
...
export async function makeRequest<T>({ url, init }: { url: string; init: RequestInit }) {
